  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <Toggle
          label="Show Bio"
          checked={selectedBlock.props.showBio ?? true}
          onChange={(checked: boolean) => handlePropChange('showBio', checked)}
        />

        {(selectedBlock.props.showBio ?? true) && (
          <Textarea
            label="Bio"
        ...
        <Toggle
          label="Show Location"
          checked={selectedBlock.props.showLocation ?? true}
          onChange={(checked: boolean) => handlePropChange('showLocation', checked)}
        />

        {(selectedBlock.props.showLocation ?? true) && (
          <Input
            label="Location"
        ...
        <Toggle
          label="Show Email"
          checked={selectedBlock.props.showEmail ?? true}
          onChange={(checked: boolean) => handlePropChange('showEmail', checked)}
        />

        {(selectedBlock.props.showEmail ?? true) && (
          <Input
            label="Email"
            value={selectedBlock.props.email || ''}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handlePropChange('email', e.target.value)}
      ...
      <div className="space-y-3">
        ...
        <Toggle
          label="Show Avatar"
          checked={selectedBlock.props.showAvatar ?? true}
          onChange={(checked: boolean) => handlePropChange('showAvatar', checked)}
        />

        {/* Avatar Preview */}
        {(selectedBlock.props.showAvatar ?? true) && (
          <>
            <button
      ...
      <div className="space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  key={option.value}
                  onClick={() => handlePropChange('avatarPosition', option.value)}
                  className={`flex-1 px-2 py-1.5 rounded-lg font-medium text-xs transition-all ${
                    (selectedBlock.props.avatarPosition || 'top') === option.value
          ...
          <div>
            ...
            <div className="flex gap-2">
              {['left', 'center', 'right'].map((align) => (
                <button
                  key={align}
                  onClick={() => handlePropChange('mainLayoutAlignment', align)}
                  className={`flex-1 px-2 py-1.5 rounded-lg font-medium text-xs transition-all ${(selectedBlock.props.mainLayoutAlignment || 'left') === align
                    ? 'bg-neutral-900 dark:bg-white text-white dark:text-neutral-900'
                    : 'bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300'
                    }`}
                >
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {['left', 'center', 'right'].map((align) => (
                <button
                  key={align}
                  onClick={() => handlePropChange('bioAlignment', align)}
                  className={`flex-1 px-2 py-1.5 rounded-lg font-medium text-xs transition-all ${
                    (selectedBlock.props.bioAlignment || selectedBlock.props.contentAlignment || 'left') === align
                      ? 'bg-neutral-900 dark:bg-white text-white dark:text-neutral-900'
                      : 'bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300'
                  }`}
                >
          ...
          <div className="space-y-2">
            ...
            <div>
              <label className="flex justify-between text-xs text-neutral-600 dark:text-neutral-400 mb-1">
                <span>Bio Max Width</span>
                <span className="font-medium text-neutral-900 dark:text-white">{selectedBlock.props.bioMaxWidth || 'Full'}px</span>
              </label>
              <input
                type="range"
                min="0"
                max="800"
                step="10"
                value={selectedBlock.props.bioMaxWidth || 0}
                onChange={(e) => handlePropChange('bioMaxWidth', parseInt(e.target.value))}
                className="w-full h-1.5 bg-neutral-200 dark:bg-neutral-700 rounded appearance-none cursor-pointer accent-neutral-900 dark:accent-white"
              />
            </div>          </div>
        </div>
